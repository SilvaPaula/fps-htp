---
title: "Mixed-models with subsampling from HTP - NDVI"
date: "`r Sys.Date()`"
...
```{r, warning=FALSE}
## Load plot table
setwd("~/Documents/PhD_KSU/PhD_Documments/POLAND_LAB/LAB_Papers/XWang et al, 2019/20190601/NDVI")
plot <- read.csv("plot_18BYD2.csv", header=TRUE, check.names=FALSE)
## convert variables to factors
plot <- plot %>%
  as.tibble() %>%
  mutate_at(vars(plot_id:treatment),as.factor)
plot <- plot[,c(1:4,7,11)]
...
ortho1 <- merge(plot, ortho1, by='plot_id')

asr <- asreml(fixed = mean ~ 1 , random =  ~id(entry) + ~id(rep) + ~id(rep):(row) + ~id(rep):(column), data=ortho1, maxit=20, subset=trt=='Untreated') 
summary(asr)$varcomp
...
ortho2 <- merge(plot, ortho2, by='plot_id')

asr <- asreml(fixed = mean ~ 1 , random =  ~id(entry) + ~id(rep) + ~id(rep):(row) + ~id(rep):(column), data=ortho2, maxit=20, subset=trt=='Untreated') 
summary(asr)$varcomp
...
ortho3 <- merge(plot, ortho3, by='plot_id')

asr <- asreml(fixed = mean ~ 1 , random =  ~id(entry) + ~id(rep) + ~id(rep):(row) + ~id(rep):(column), data=ortho3, maxit=20, subset=trt=='Untreated') 
summary(asr)$varcomp
...
ortho4 <- merge(plot, ortho4, by='plot_id')

asr <- asreml(fixed = mean ~ 1 , random =  ~id(entry) + ~id(rep) + ~id(rep):(row) + ~id(rep):(column), data=ortho4, maxit=20, subset=trt=='Untreated') 
summary(asr)$varcomp
...
ortho5 <- merge(plot, ortho5, by='plot_id')

asr <- asreml(fixed = mean ~ 1 , random =  ~id(entry) + ~id(rep) + ~id(rep):(row) + ~id(rep):(column), data=ortho5, maxit=20, subset=trt=='Untreated') 
summary(asr)$varcomp
...
ortho6 <- merge(plot, ortho6, by='plot_id')

asr <- asreml(fixed = mean ~ 1 , random =  ~id(entry) + ~id(rep) + ~id(rep):(row) + ~id(rep):(column), data=ortho6, maxit=20, subset=trt=='Untreated') 
summary(asr)$varcomp
...
photo5 <- merge(plot, photo5, by='plot_id', suffixes = c("", ".y"))
plot.mean5 <- aggregate(photo5$mean, list(photo5$plot_id), mean)
colnames(plot.mean5) <- c('plot_id', 'mean')
plot.mean5 <- merge(plot, plot.mean5)

asr <- asreml(fixed= mean ~ 1 , random = ~id(entry) + ~id(rep) + ~id(rep):(row) + ~id(rep):(column), data=plot.mean5, maxit=60, subset= trt=='Untreated')
summary(asr)$varcomp
...
photo6 <- merge(plot, photo6, by='plot_id', suffixes = c("", ".y"))
plot.mean6 <- aggregate(photo6$mean, list(photo6$plot_id), mean)
colnames(plot.mean6) <- c('plot_id', 'mean')
plot.mean6 <- merge(plot, plot.mean6)

asr <- asreml(fixed= mean ~ 1 , random = ~id(entry) + ~id(rep) + ~id(rep):(row) + ~id(rep):(column), data=plot.mean6, maxit=60, subset= trt=='Untreated')
summary(asr)$varcomp
...
```{r}
## 2018-04-23
asr <- asreml(fixed = mean ~ 1, random =  ~id(entry) + ~id(rep) + ~id(rep):(row) + ~id(rep):(column) + ~id(rep):(row):(column):(entry), data=photo5, maxit=20, subset= trt=='Untreated') 
summary(asr)$varcomp
...
```{r}
## 2018-05-16
asr <- asreml(fixed = mean ~ 1, random =  ~id(entry) + ~id(rep) + ~id(rep):(row) + ~id(rep):(column) + ~id(rep):(row):(column):(entry), data=photo6, maxit=20, subset= trt=='Untreated') 
summary(asr)$varcomp
...
```{r}
## 2018-04-23
asr = asreml(fixed= mean ~ 1, random =  ~id(entry) + ~id(rep) + ~id(original_file), data=photo5, maxit=60, subset= trt=='Untreated')
summary(asr)$varcomp
...
Vi <- getVariance(asr, 'original_file')
Ve <- getVariance(asr, 'units!R')
h45 <- round(Vg/(Vg + Vi/(reps) + Ve/(reps*hm)),3) # heritability
...
```{r}
## 2018-05-16
asr = asreml(fixed= mean ~ 1, random =  ~id(entry) + ~id(rep) + ~id(original_file), data=photo6, maxit=60, subset= trt=='Untreated')
summary(asr)$varcomp
...
Vi <- getVariance(asr, 'original_file')
Ve <- getVariance(asr, 'units!R')
h46 <- round(Vg/(Vg + Vi/(reps) + Ve/(reps*hm)),3) # heritability
...
```{r}
## 2018-03-30
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho1, maxit=20)
summary(asr)$varcomp

## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho1, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=ortho1, maxit=20, G.param=iv)
summary(asr)$varcomp
...
```{r}
## 2018-04-04
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho2, maxit=20)
summary(asr)$varcomp

## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho2, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=ortho2, maxit=20) ## If G.param = iv is specified: Error in asreml(fixed = mean ~ 1 + trt, random = ~id(entry): Singularities in the average information matrix.
summary(asr)$varcomp
...
```{r}
## 2018-04-12
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho3, maxit=20)
summary(asr)$varcomp

## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho3, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=ortho3, maxit=20, G.param=iv)
summary(asr)$varcomp
...
```{r}
## 2018-04-19
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho4, maxit=20)
summary(asr)$varcomp

## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho4, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=ortho4, maxit=20, G.param=iv)
summary(asr)$varcomp
...
```{r}
## 2018-04-23
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho5, maxit=20)
summary(asr)$varcomp

## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho5, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=ortho5, maxit=20, G.param=iv)
summary(asr)$varcomp

Vg <- getVariance(asr, 'entry')
Vd <- getVariance(asr, 'trt:entry')
Ve <- getVariance(asr, 'units!R')
h55 <- round(Vg/(Vg + Vd/trts + Ve/(reps*trts)),3) # heritability
...
```{r}
## 2018-05-16
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) , data=ortho6, maxit=20)
summary(asr)$varcomp

Vg <- getVariance(asr, 'entry')
Vd <- getVariance(asr, 'trt:entry')
Ve <- getVariance(asr, 'units!R')
h56 <- round(Vg/(Vg + Vd/trts + Ve/(reps*trts)),3) # heritability
...
## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean1, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean1, maxit=20, G.param=iv)
summary(asr)$varcomp
...
## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean2, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean2, maxit=20) ## If G.param = iv is specified: Error in asreml(fixed = mean ~ 1 + trt, random = ~id(entry): Singularities in the average information matrix.
summary(asr)$varcomp
...
## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean3, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean3, maxit=20, G.param=iv)
summary(asr)$varcomp
...
## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean4, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean4, maxit=20, G.param=iv)
summary(asr)$varcomp
...
```{r}
## 2018-04-23
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean5, maxit=20)
summary(asr)$varcomp

## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean5, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean5, maxit=20, G.param=iv)
summary(asr)$varcomp

Vg <- getVariance(asr, 'entry')
Vd <- getVariance(asr, 'trt:entry')
Ve <- getVariance(asr, 'units!R')
h65 <- round(Vg/(Vg + Vd/trts + Ve/(reps*trts)),3) # heritability
...
```{r}
## 2018-05-16
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean6, maxit=20)
summary(asr)$varcomp

## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean6, maxit=20, start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random = ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column), data=plot.mean6, maxit=20, G.param=iv)
summary(asr)$varcomp

Vg <- getVariance(asr, 'entry')
Vd <- getVariance(asr, 'trt:entry')
Ve <- getVariance(asr, 'units!R')
h66 <- round(Vg/(Vg + Vd/trts + Ve/(reps*trts)),3) # heritability
...
## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo1, maxit=60, workspace='2gb', start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo1, maxit=60, workspace='2gb', G.param=iv)
summary(asr)$varcomp
...
## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo2, maxit=60, workspace='2gb', start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo2, maxit=60, workspace='2gb') ## If G.param = iv is specified: Error in asreml(fixed = mean ~ 1 + trt, random = ~id(entry): Singularities in the average information matrix.
summary(asr)$varcomp
...
## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo3, maxit=60, workspace='2gb', start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo3, maxit=60, workspace='2gb') ## If G.param = iv is specified: Error in asreml(fixed = mean ~ 1 + trt, random = ~id(entry): Singularities in the average information matrix.
summary(asr)$varcomp
...
```{r}
## 2018-04-19
asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo4, maxit=60, workspace='2gb')
summary(asr)$varcomp

## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo4, maxit=60, workspace='2gb', start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo4, maxit=60, workspace='2gb', G.param=iv)
summary(asr)$varcomp
...
```{r}
## 2018-04-23
asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo5, maxit=60, workspace='2gb')
summary(asr)$varcomp

## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo5, maxit=60, workspace='2gb', start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo5, maxit=60, workspace='2gb', G.param=iv)
summary(asr)$varcomp
...
Vd <- getVariance(asr, 'trt:rep:row:column:entry')
Ve <- getVariance(asr, 'units!R')
h75 <- round(Vg/(Vg + Vt/trts + Vd/(reps*trts) + Ve/(reps*trts*hm)),3) # heritability
...
```{r}
## 2018-05-16
asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo6, maxit=60, workspace='2gb')
summary(asr)$varcomp

## Starting values and unconstrained variances
asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo6, maxit=60, workspace='2gb', start.values = TRUE) 
iv <- asr$vparameters.table
iv
iv$Constraint[2]='U'
iv$Constraint[3]='U'

asr <- asreml(fixed = mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(trt):(rep) + ~id(trt):(rep):(row) + ~id(trt):(rep):(column) + ~id(trt):(rep):(row):(column):(entry), data=photo6, maxit=60, workspace='2gb', G.param=iv)
summary(asr)$varcomp
...
Vd <- getVariance(asr, 'trt:rep:row:column:entry')
Ve <- getVariance(asr, 'units!R')
h76 <- round(Vg/(Vg + Vt/trts + Vd/(reps*trts) + Ve/(reps*trts*hm)),3) # heritability
...
```{r}
## 2018-04-23
asr = asreml(fixed= mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(rep) + ~id(original_file), data=photo5, maxit=60, workspace='2gb')
summary(asr)$varcomp
...
Vi <- getVariance(asr, 'original_file')
Ve <- getVariance(asr, 'units!R')
h85 <- round(Vg/(Vg + Vi/(reps) + Ve/(reps*hm)),3) # heritability
...
```{r}
## 2018-05-16
asr = asreml(fixed= mean ~ 1 + trt, random =  ~id(entry) + ~id(trt):(entry) + ~id(rep) + ~id(original_file), data=photo6, maxit=60, workspace='2gb')
summary(asr)$varcomp
...
Vi <- getVariance(asr, 'original_file')
Ve <- getVariance(asr, 'units!R')
h86 <- round(Vg/(Vg + Vi/(reps) + Ve/(reps*hm)),3) # heritability
...
|------------|-----------------------|-----------------------|--------------------------|-------------------------------|
| 2018-03-30 | `r h11` - `r avsed11` | `r h21` - `r avsed21` |   `r h31` - `r avsed31`  |     `r h41` - `r avsed41`     |
| 2018-04-04 | `r h12` - `r avsed12` | `r h22` - `r avsed22` |   `r h32` - `r avsed32`  |     `r h42` - `r avsed42`     | 
| 2018-04-12 | `r h13` - `r avsed13` | `r h23` - `r avsed23` |   `r h33` - `r avsed33`  |     `r h43` - `r avsed43`     | 
| 2018-04-19 | `r h14` - `r avsed14` | `r h24` - `r avsed24` |   `r h34` - `r avsed34`  |     `r h44` - `r avsed44`     | 
| 2018-04-23 | `r h13` - `r avsed13` | `r h23` - `r avsed23` |   `r h33` - `r avsed33`  |     `r h43` - `r avsed43`     | 
| 2018-05-16 | `r h14` - `r avsed14` | `r h24` - `r avsed24` |   `r h34` - `r avsed34`  |     `r h44` - `r avsed44`     | 
...
| 2018-04-23 | `r h55` - `r avsed55` | `r h65` - `r avsed65` |    `r h75` - r avsed75  |     `r h85` - `r avsed85`     | 
| 2018-05-16 | `r h56` - `r avsed56` | `r h66` - `r avsed66` |    `r h76` - r avsed76  |     `r h86` - `r avsed86`     |
